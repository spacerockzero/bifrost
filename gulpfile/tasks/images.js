...
var isNotRaw = function(file){
  // console.log('file.path, isNotRaw:',file.path, path.basename(file.path).indexOf('-raw') === -1);
  return path.basename(file.path).indexOf('-raw') === -1;  // return (if file has "-raw" as filename suffix) boolean
};
...
gulp.task('img', ['clean'], function() {
  ...
  return gulp.src( imgSrc, { matchBase: true } )
    // if files have "-raw" in filename, don't optimize them
    // use either gulp-if or gulp-filter here
    .pipe( gulpif( isNotRaw, imagemin(minConfig) ) )
    .pipe( map(log) ) // log files going to dist
    .pipe( gulp.dest( imgDest ) );
});